...
if(!isset($_POST['sitename']) || !isset($_POST['url']) || !isset($_POST['description']) || !isset($_POST['rate']))
{
	echo json_encode(array("error"=>true, "error_text"=>"Не верный формат данных"));
	die();
}

if (!($stmt = $mysqli->prepare("INSERT INTO sites (name, url, description, rate) VALUES (?, ?, ?, ?)")))
{
	echo json_encode(array("error"=>true, "error_text"=>"Не удалось подготовить запрос: (" . $mysqli->errno . ") " . $mysqli->error));
	die();
}

if(!$stmt->bind_param("sssi", $_POST['sitename'], $_POST['url'], $_POST['description'], $_POST['rate']))
{
